...
@bind steps Slider(0:100, show_value=true)

# ╔═╡ 0e3494fa-4988-4de0-befc-626caef9140a
@bind interval Slider(0.1:0.01:2, show_value=true)

# ╔═╡ 5c7a47cc-268c-4da1-8eac-d1c8bb222342
@bind frequency Slider(1:10, show_value=true)

# ╔═╡ f809356e-3343-4119-9b34-e89c38bc28fa
exp.((im*2π*frequency/interval).*(0:steps))

# ╔═╡ 6be14323-6cfc-4194-8b4c-b8b22e7a7388
begin
	points = exp.(exp.((im*2π*frequency/interval).*(0:steps)))
	scatter()
	for point in points
		scatter!([real(point)], [imag(point)])
	end
	xlims!(-1, 1)
	ylims!(-1, 1)
end

# ╔═╡ 32d1a138-8d20-445c-8f37-6fea306c53ed
2π*im
...
manifest_format = "2.0"
project_hash = "3f14ad5f1038976e1bfcd5e2056a8c916546a363"
